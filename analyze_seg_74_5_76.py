...
# -*- coding: utf-8 -*-
"""
Универсальный анализатор результатов VanitySearch (out_*.txt).

Назначение:
- Анализировать файлы результатов VanitySearch (где блоки начинаются с 'PubAddress:')
...
Важно:
- Файл специально оставлен с историческим именем, чтобы его можно было импортировать из GUI.
"""
...
def load_results(cfg: AnalyzerConfig) -> tuple[list[dict[str, str]], dict[str, int], list[Path]]:
    ...
    files = sorted(base_dir.glob(cfg.glob_pattern))
    if cfg.max_files is not None:
        files = files[: max(0, cfg.max_files)]

    all_results: list[dict[str, str]] = []
    file_stats: dict[str, int] = {}
    for fp in files:
        parsed = parse_result_file(fp)
        file_stats[fp.name] = len(parsed)
        all_results.extend(parsed)
        if cfg.max_records is not None and len(all_results) >= cfg.max_records:
            all_results = all_results[: cfg.max_records]
            break

    return all_results, file_stats, files
...
def render_report(analysis: dict[str, Any]) -> str:
    ...
    hdr("ANALYZER: VanitySearch out-файлы")
    lines.append(f"Base dir:   {cfg.base_dir}")
    lines.append(f"Glob:       {cfg.glob_pattern}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Универсальный анализатор результатов VanitySearch")
    parser.add_argument(
        "--base-dir",
        default=str(Path(__file__).parent / "runs_cpu_gui"),
        help="Папка с out-файлами",
    )
    parser.add_argument("--glob", default="out_*.txt", help="Маска файлов (glob) внутри base-dir")
    ...
    parser.add_argument("--max-files", default="", help="Ограничить число файлов (опционально)")
    ...
    report = generate_report(
        ...
        puzzle_bits=puzzle_bits,
        max_files=_safe_int(args.max_files) if str(args.max_files).strip() else None,
