#!/bin/bash
# GPU поиск на сервере: window_avg (из ANALYZE.txt), узкое окно вокруг 87.9978%
# 120 сегментов (чередование up/down), паттерны через -i + базовый PATTERN без wildcard для вывода Prob
...
PATTERNS_FILE="patterns_target_pack.txt"   # для -i (см. VanitySearch/ANALYZE.txt -> TARGET PACK)
PATTERN="1PWo3JeB"  # БЕЗ wildcard — VanitySearch печатает Prob для такого префикса
BITS=71
...
CPU_THREADS=2  # На GPU обычно хватает 1-4 (лишние CPU потоки могут мешать системе)
MAXFOUND=10000000  # Увеличенный буфер для предотвращения переполнения (10 миллионов)
AUTOSAVE_INTERVAL=60  # Автосохранение каждые 1 минут

echo "╔════════════════════════════════════════════════════════════════════════════╗"
echo "║  🎯 УЗКИЙ ПОИСК PUZZLE #71 (window_avg: 87.6978%..88.2978%)                ║"
echo "║  120 сегментов (up/down), паттерны: -i + базовый префикс                   ║"
...
echo "PATTERNS_FILE=$PATTERNS_FILE (для -i)"
...
if [ ! -f "$SEG_FILE" ]; then
    ...
    echo "   Скопируйте $SEG_FILE на сервер"
    exit 1
fi

# Проверка файла паттернов
if [ ! -f "$PATTERNS_FILE" ]; then
    echo "❌ Ошибка: файл паттернов $PATTERNS_FILE не найден!"
    echo "   Скопируйте $PATTERNS_FILE на сервер"
    exit 1
fi
...
echo ""
echo "6. 🎯 Этот диапазон УЗКИЙ (~0.60%):"
echo "      Основано на анализе (ANALYZE.txt): окно window_avg вокруг средней Puzzle-координаты"
echo ""
