...
**⚠️ ВАЖНО:** `PuzzleKeyAbs` — это **позиция в puzzle-диапазоне ДО применения эндоморфизма/симметрии**, а не реальный приватный ключ!  
**НЕ пытайтесь конвертировать `PuzzleKeyAbs` напрямую в адрес** — вы получите неправильный адрес.  
Для конвертации в адрес используйте **`Priv (HEX)`** — это реальный приватный ключ после всех оптимизаций.
...
- **для конвертации в адрес используйте `Priv (HEX)`, а НЕ `PuzzleKeyAbs`** — они могут отличаться из-за эндоморфизма

**Почему `PuzzleKeyAbs` ≠ `Priv (HEX)`?**
VanitySearch использует оптимизации (эндоморфизм кривой secp256k1 и симметрию точек), которые позволяют ускорить поиск.  
`PuzzleKeyAbs` и `SegKey` — это ключи **до применения этих оптимизаций** (позиция в перебираемом диапазоне).  
`Priv (HEX)` — это **реальный приватный ключ после оптимизаций**, который соответствует найденному адресу.  
Оба ключа математически эквивалентны (дают один и тот же публичный ключ), но имеют разные числовые значения.
