...
#include <sqlite3.h>
#if defined(_MSC_VER)
  // MSVC deprecates POSIX strdup() in favor of _strdup().
  #define VS_STRDUP _strdup
#else
  #define VS_STRDUP strdup
#endif
#ifndef WIN64
#include <pthread.h>
#endif
        ...
        for (int j = 0; j < (int)subList.size(); j++) {
          if (suffixStr.empty()) {
            if (initPrefix(subList[j], &it)) {
              it.found = found;
              it.prefix = VS_STRDUP(it.prefix); // We need to allocate here, subList will be destroyed
              it.suffix = NULL;
              it.suffixLength = 0;
            ...
            if (initPrefixSuffix(subList[j], suffixStr, &it)) {
              it.found = found;
              it.prefix = VS_STRDUP(it.prefix);
              it.suffix = VS_STRDUP(it.suffix);
              itPrefixes.push_back(it);
            }
          }
        }
